...
- 音质可选低、中、高，默认高音质。
- 输出音量可通过滑块在 `0%–200%` 之间调节，步进 `5%`，默认 `100%`；该设置作用于整个歌单的最终 MP4 音频。`100%` 保持音视频流直接复制，其他音量只重编码音频，视频画面不会重新渲染。
- 不同输出音量使用相互隔离的歌曲缓存，避免切换音量后错误复用另一音量的生成缓存。
...
```text
歌单名_高/中/低_质量/平衡/极速_AAAAxBBBB_XFPS_VOL音量.mp4
```

例如：

```text
我的歌单_高_极速_1920x1080_1FPS_VOL100.mp4
我的歌单_高_平衡_1920x1080_1FPS_VOL150.mp4
我的歌单_中_质量_1600x900_15FPS_VOL80.mp4
```
