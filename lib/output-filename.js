...
function playlistOutputIdentity(options = {}) {
  ...
  const modeKey = mode === 'fast' ? 'fast' : (mode === 'lite_video' ? 'lite' : 'default');
  ...
  return `${sourceKey}_${profile.quality}_${modeKey}_${profile.resolution}_${profile.fps}fps_${orderKey}_${id}_v${Number(version) || 1}.mp4`;
}

function buildPlaylistOutputFilename(options = {}) {
  const stem = sanitizeOutputFileStem(options.name, 'playlist');
  const profile = resolveGenerationProfile(options, { allowLiteVideo: true });
  const modeLabel = profile.mode === 'fast' ? '极速' : (profile.mode === 'lite_video' ? '轻量' : '标准');
  const qualityLabel = AUDIO_QUALITIES[profile.quality].label;
  return `${stem}_${qualityLabel}_${modeLabel}_${profile.resolution}_${profile.fps}FPS.mp4`;
}
