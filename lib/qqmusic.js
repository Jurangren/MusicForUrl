...
async function getPlaylistDetail(disstid, cookie = '') {
  ...
  const tracks = (cd.songlist || []).map(t => ({
    ...
    subtitle: t.songsubtitle || t.subtitle || t.desc || '',
...
const DEFAULT_QQ_QUALITY = 'high';

async function getSongUrl(songMid, cookie = '', qualityKey = DEFAULT_QQ_QUALITY) {
  const uin = extractUin(cookie) || '0';
  const musicKey = extractMusicKey(cookie);

  const selectedQuality = Object.hasOwn(QQ_QUALITY_MAP, qualityKey) && qualityKey !== 'lossless'
    ? qualityKey
    : DEFAULT_QQ_QUALITY;
  const quality = QQ_QUALITY_MAP[selectedQuality];
...
async function getSongDetail(songMid, cookie = '') {
  const params = new URLSearchParams({
  ...
  const cookieStr = normalizeCookie(cookie);
  const res = await qqFetch(`https://c.y.qq.com/v8/fcg-bin/fcg_play_single_song.fcg?${params.toString()}`, {
    headers: { ...(cookieStr ? { Cookie: cookieStr } : {}) }
  });
  if (!res.ok) throw new Error(`QQ Music API HTTP ${res.status}`);
  const data = parseQQJsonPayload(await res.text());
  const track = Array.isArray(data?.data) ? data.data[0] : data?.data;
  if (!track) throw new Error(data?.message || data?.msg || '获取 QQ 音乐单曲详情失败');

  const mid = String(track.songmid || track.mid || songMid);
  return {
    id: mid,
    songId: track.songid || track.id,
    name: track.songname || track.name || track.title || '',
    subtitle: track.songsubtitle || track.subtitle || track.desc || '',
    album: track.albumname || track.album?.name || '',
    artist: getArtists(track.singer),
    duration: getDurationSeconds(track.interval || track.duration || 0),
    cover: getCoverUrl(track.album || { mid: track.albummid }),
    mid,
    mediaMid: track.strMediaMid || track.file?.media_mid || mid
  };
}

async function getLyrics(songMid, cookie = '') {
  const params = new URLSearchParams({
    songmid: String(songMid),
    format: 'json',
    nobase64: '1',
  ...
  try {
    const cookieStr = normalizeCookie(cookie);
    const res = await qqFetch(`https://c.y.qq.com/lyric/fcgi-bin/fcg_query_lyric_new.fcg?${params.toString()}`, {
      headers: { ...(cookieStr ? { Cookie: cookieStr } : {}) }
    });
    const text = await res.text();
    const data = parseQQJsonPayload(text);
    const normalizeLyricField = (value) => {
      let lyricText = String(value || '');
      if (lyricText && !lyricText.includes('[') && /^[A-Za-z0-9+/=\s]+$/.test(lyricText)) {
        try { lyricText = Buffer.from(lyricText, 'base64').toString('utf8'); } catch (_) {}
      }
      return lyricText
        .replace(/&#58;/g, ':')
        .replace(/&#46;/g, '.')
    ...
    return {
      original: normalizeLyricField(data?.lyric),
      translation: normalizeLyricField(data?.trans)
    };
  } catch (error) {
    console.warn(`[QQ歌词] 获取失败 songMid=${songMid}:`, error?.message || error);
